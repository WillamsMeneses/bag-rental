...
import { PaymentResultDrawer } from '@/components/sections/rentals/PaymentResultDrawer';
...
export const ListingDetailPage: React.FC = () => {
  ...
  const [paymentResult, setPaymentResult] = useState<'success' | 'failed' | 'processing' | null>(
    location.state?.paymentResult ??
    (searchParams.get('paymentCancelled') ? 'failed' : null)
  );
  ...
  const listingLocation = [listing.city, listing.state].filter(Boolean).join(', ');
  ...
  return (
    <>
      <PaymentResultDrawer
        ...
        onKeepLooking={() => { setPaymentResult(null); navigate('/'); }}
        onViewRentals={() => { setPaymentResult(null); navigate('/my-rentals'); }}
              ...
              {location && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1, borderBottom: '0.5px solid', borderColor: 'grey.100' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{listingLocation}</Typography>
                  </Box>
                </Box>
              )}
